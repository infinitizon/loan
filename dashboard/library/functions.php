...
function doLogin() {
    ...
    $sql = "SELECT u.id,u.fname, u.lname, u.email, u.due, u.loanstat, u.gender, u.dob, u.is_active, u.pics, u.phone,
			a.acc_no, a.user_id, a.pin, a.type, a.status,
			ad.address, ad.city, ad.state,  ad.marital, ad.company,ad.occupation,ad.empduration,ad.emplphone,ad.monthlyincome,ad.purpose,ad.loancategory,
			ad.lamount, ad.repayment,ad.homeowner, ad.creditstatus, ad.taxid , ad.ssn , ad.loandate, ad.zipcode
...
function doRegister() {
	...
	$creditstatus =  $_POST['creditstatus'];
	$taxid =  $_POST['taxid'];
	$ssn =  $_POST['ssn'];
...
//	$accno 	= (int)$_POST['accno'];
    ...
    $insert_id = 0;
    $sql = "INSERT INTO tbl_users (fname, lname, pwd, email, due, loanstat, phone, gender, dob, is_active, utype, pics, bdate)
			VALUES ('$fname', '$lname', PASSWORD('$pwd'), '$email', '$due', '$loanstat', '$phone', '$gender', '$dob', 'FALSE','USER', '$thumbnail', NOW())";
    dbQuery($sql);
    $insert_id = dbInsertId();

    //now create a user address. 
    $sql = "INSERT INTO tbl_address (user_id, address, city, state, country , marital, occupation,company, empduration, emplphone,monthlyincome, purpose , 
	loancategory  ,lamount , repayment ,homeowner, creditstatus, taxid , ssn  , loandate , zipcode ) 
			VALUES ($insert_id, '$add', '$city', '$state', '$country', '$marital', '$occupation', '$company', '$empduration', '$emplphone', '$monthlyincome', 
			'$purpose', '$loancategory', '$lamount','$repayment','$homeowner', '$creditstatus','$taxid','$ssn', '$loandate',$zip)";
    dbQuery($sql);
    ...
    dbQuery($sql);
	     ...
	     //now send email
    //email it now.	
    $subject = "Cognate Bank Account Created";
    $to = $email;
	...
	This is to inform you that your Account # $accno is register successfully with Country Mortgage & Loan LTD currently in Inactive state.<br/><br/> Country Mortgage & Loan LTD received your application, a
representative will get back to you shortly<br/><br/>
	Regards,<br/>Country Mortgage & Loan LTD";
