...
function initiateTransferFunds()
{
    ...
    if (dbNumRows($results) > 0) {
        ...
        if ($status == 'INACTIVE') {
            $msg = '';
            header('Location: index.php?v=sucess');
            exit();
        }
    }
...
function transferFunds() 
{
	...
	if (dbNumRows($result) == 1) {
		...
		$msg = 'Receivers account number does not exist or not active. Please contact custmer care.';
...
function funds_transfer_mail($amt, $sacc_no) {
	...
	$msg_body = sprintf("Dear Customer,<br/><br/>
		...
		Funds Transfer on %s using Bank Transfer System.<br/><br/>In case you need any further clarification for the same, 
		please do get in touch with your Home Branch.<br/><br/>
		Regards,<br/>Thanks For Banking with us", 
